...
public class ZLMHttpHookListener {
	 ...
	 */
	@ResponseBody
	@PostMapping(value = "/on_file_upload", produces = "application/json;charset=UTF-8")
	public ResponseEntity<String> onFileUpload(HttpServletRequest request, @RequestBody JSONObject jsonObject){
		if (logger.isDebugEnabled()) {
			logger.debug("ZLM HOOK on_file_upload API调用，参数：" + jsonObject.toString());
		}
		...
		List<ZLMHttpHookSubscribe.Event> subscribes = this.subscribe.getSubscribes(ZLMHttpHookSubscribe.HookType.on_file_upload);
		if (subscribes != null  && subscribes.size() > 0) {
			for (ZLMHttpHookSubscribe.Event subscribe : subscribes) {
				MediaServerItem mediaServerItem = new MediaServerItem();
				mediaServerItem.setStreamId(streamId);
				subscribe.response(mediaServerItem, jsonObject);
			}
		}
		JSONObject ret = new JSONObject();
		ret.put("code", 0);
		...
		if ("rtp".equals(app)){
			...
			if (streamInfoForPlayCatch != null) {
				if (redisCatchStorage.isChannelSendingRTP(streamInfoForPlayCatch.getChannelId())) {
					...
					cmder.streamByeCmd(VideoManagerConstants.VIDEO_PREVIEW,streamInfoForPlayCatch.getDeviceID(), streamInfoForPlayCatch.getChannelId());
				...
				if (streamInfoForPlayBackCatch != null) {
					cmder.streamByeCmd(VideoManagerConstants.VIDEO_PLAYBACK,streamInfoForPlayBackCatch.getDeviceID(), streamInfoForPlayBackCatch.getChannelId());
